...
PUA owns the complete Provider prompt and sends AgentHub a schema-v2 message containing that prompt as `text` plus a `pua.resource-message.v1` opaque payload. The payload retains the original text, role, sender, Workspace instance, type, and causation for PUA history and recovery; AgentHub forwards only the top-level text without interpreting PUA metadata. The timeline decodes the payload and shows the original message with its sender. Browser user names remain local provenance, with missing or invalid names falling back to `User`.
