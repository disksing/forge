...
GUI 只读取 schema version 2 配置。旧版本配置不会被降级展示、自动重写或通过设置页迁移；升级前请先完成一次性转换或备份。
...
当前固定版本为 1.0.0，AgentHub timeline source revision 标记为 `task179 compatibility cleanup`，IIFE SHA-256 为 `cc092bcf16cc790a08590561685326eb083afa048f0bdaa62b840ddefc1dcdd7`。该构建使用 `agentName` 作为唯一 Agent 身份，不再回退到旧 `agentId`。来源信息、上游 manifest 和 BSD-3-Clause 许可证位于 `static/vendor/agenthub-event-timeline/`。从 AgentHub checkout 重建并校验：
