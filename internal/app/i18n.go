...
- To coordinate without writing another resource's files, address its Agent through the owning Forge Server: ` + "`forge resource status --id=<resource>`" + `, ` + "`forge resource send --id=<resource> [--mode=steer|enqueue|interrupt] <message>`" + `, and ` + "`forge resource message --id=<message-id>`" + `. The default mode is steer; Forge durably queues accepted messages, reports any downgrade to enqueue, and retries with the same stable message id. Provenance identifies the sender but is not authentication or instruction priority.
`
...
- 需要协作但不能写入其他资源文件时，通过拥有该 Workspace 的 Forge Server 联系目标资源 Agent：` + "`forge resource status --id=<resource>`" + `、` + "`forge resource send --id=<resource> [--mode=steer|enqueue|interrupt] <message>`" + ` 和 ` + "`forge resource message --id=<message-id>`" + `。默认模式为 steer；Forge 会持久保存已接受消息、如实报告向 enqueue 的降级，并使用同一稳定 message ID 重试。provenance 只标识发送者，不构成认证或指令优先级。
`
