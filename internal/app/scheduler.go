...
func defaultSchedulerMarkdown(language string) string {
	if language == languageSimplifiedChinese {
		return "# Scheduler 上下文\n\n由 Scheduler Agent 维护长期判断上下文。具体调度项保存在 scheduler.json 中，并通过 Forge CLI 修改。\n"
	}
	return "# Scheduler Context\n\nMaintained by the Scheduler Agent for durable judgment context. Schedules live in scheduler.json and are changed through the Forge CLI.\n"
}
...
- scheduler.md 用于你维护长期判断上下文；需要记录上次检查、触发、补跑、重复或时区判断时写在这里。
...
- Maintain durable judgment context in scheduler.md, including prior checks, triggers, catch-up, repetition, or timezone decisions when useful.
