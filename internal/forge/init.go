...
- ` + "`forge start [--project=<project>] [--task=<task>] [-- <agent command...>]`" + ` is a plain Session launcher. It creates a session, locks the selected resource, runs the agent, and ends the session; it does not schedule or update Self-Driving.
`
