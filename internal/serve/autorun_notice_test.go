...
func TestBeginSchedulerTurnAssignsStableBoundaries(t *testing.T) {
	run := agentRun{ResourceID: "project1.task1", AutoRunGeneration: 4}
...
func TestAutoRunFinishNoticeCarriesScopedLifecycle(t *testing.T) {
	runID := "run-notice"
	...
	rt := &agentRuntime{run: agentRun{
		ID: runID, ResourceID: "project1.task1", AutoRunGeneration: 7,
		SchedulerTurnID: "turn-2", SchedulerTurnSequence: 2,
	}}
	rt.addAutoRunFinishNotice(manager, "info", autoRunFinishNoticeWaitingLifecycle, "waiting")
	message := <-channel
	if message.Notice == nil {
		t.Fatal("finish notice was not published")
	}
	data := message.Notice.Data
	if data.Kind != autoRunFinishNoticeKind || data.Lifecycle != autoRunFinishNoticeWaitingLifecycle ||
		data.RunID != runID || data.ResourceID != "project1.task1" || data.AutoRunGeneration != 7 ||
		data.SchedulerTurnID != "turn-2" || data.SchedulerTurnSequence != 2 {
		t.Fatalf("finish notice is missing lifecycle scope: %+v", data)
	}
}

func TestAutoRunFinishNoticeFrontendLifecycle(t *testing.T) {
	node, err := exec.LookPath("node")
	if err != nil {
		t.Skip("node is required for the AutoRun notice lifecycle test")
	}
	...
	source := string(data)
	start := strings.Index(source, "function isAutoRunWaitingFinishNotice(")
	...
	if start < 0 || end < 0 || appendStart < 0 || appendEnd < 0 || streamStart < 0 || streamEnd < 0 || loadEnd < 0 {
		t.Fatal("could not isolate AutoRun notice lifecycle functions")
	}
	constants := source[strings.Index(source, "const AUTORUN_FINISH_NOTICE_KIND ="):strings.Index(source, "const AGENT_DRAFT_STORAGE_PREFIX =")]
...
const state = {
  ...
  activeRunId: "run-a",
  tree: { projects: [{ id: "project1", children: [{ id: "task1", autoRun: { generation: 7, state: "suspended" } }] }] },
  details: {},
  agentRunProjectionVersion: 0,
  agent: {
    activeRunId: "run-a",
    runs: [{ id: "run-a", resourceId: "task1", autoRunGeneration: 7, schedulerTurn: false, schedulerTurnId: "turn-1", schedulerTurnSequence: 1 }],
    notices: [],
...
const waiting = (sequence, turnId = "turn-" + sequence) => ({
  source: "forge", type: "forge.notice", data: {
    level: "info", method: "forge/autorun/finish", kind: "autorun-finish", lifecycle: "until-resume",
    runId: "run-a", resourceId: "task1", autoRunGeneration: 7,
...
const finishError = { source: "forge", type: "forge.notice", data: {
  level: "error", method: "forge/autorun/finish", kind: "autorun-finish", lifecycle: "error",
  runId: "run-a", resourceId: "task1", autoRunGeneration: 7, text: "finish failed",
} };
...
state.tree.projects[0].children[0].autoRun = { generation: 7, state: "running" };
...
state.tree.projects[0].children[0].autoRun = { generation: 7, state: "suspended" };
...
state.agent.runs[0] = { ...state.agent.runs[0], autoRunGeneration: 8, schedulerTurn: false, schedulerTurnId: "turn-3", schedulerTurnSequence: 3 };
state.tree.projects[0].children[0].autoRun = { generation: 8, state: "running" };
context.reconcileAgentNotices(state.agent.runs);
...
(async () => {
  ...
  const oldRun = { id: "run-a", resourceId: "task1", autoRunGeneration: 7, schedulerTurn: false, schedulerTurnId: "turn-1", schedulerTurnSequence: 1 };
...
`
	...
	testFile := filepath.Join(t.TempDir(), "autorun-notice-lifecycle.js")
