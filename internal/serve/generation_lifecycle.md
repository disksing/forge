...
Workspace Generation 预算同样位于 planner 外围，但不在 poller 的 Turn 终态路径中收敛。poller 先完整记录 completion 与 Task 状态收口；mailbox 只有在 queued input 即将从精确的 ready/stopped Session 开始新 Turn 时，才读取 AgentHub materialized closed Turns，重建当前 generation 的 Turn 数与累计 wall-clock。达到 Workspace 配置的任一阈值后，先持久化原因 `turn_limit` 的 replacement intent并保留该 input，再交给上述统一 planner 执行 Stop → Archive → Retire，由 successor 接管投递。stopped generation 在 Resume 前检查；活动 Turn 的 steer 不开始新 Turn，因此不检查。没有 input 时，超预算 generation 可以保持 ready 或进入 idle suspended，不产生主动轮换工作。
