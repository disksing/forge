...
for (const [resourceId, hasAutoRun] of [["project1.task1", true], ["project1", false]]) {
  for (const status of statuses) {
    ...
    assert(html.includes('id="agentStopButton"'), resourceId + " " + status + " internal lock must preserve Close Session");
...
assert(projectInternalHTML.includes('id="agentStopButton"'), "an internal project lock must preserve Close Session");
