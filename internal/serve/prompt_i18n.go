...
var englishSelfDrivingPrompts = selfDrivingPromptCatalog{
	...
	recovery:               "Recover and continue the current Self-Driving generation. Read task.md, work.md, and the relevant Self-Driving entries in log.jsonl before continuing.",
	suspended:              "This Self-Driving generation was previously suspended. Check the wake condition first. If it is satisfied, continue working. If it is not satisfied, suspend again only when no other in-scope work can make progress and continuing would only poll that external condition; otherwise keep working.",
...
var chineseSelfDrivingPrompts = selfDrivingPromptCatalog{
	...
	recovery:               "恢复并继续当前 Self-Driving generation。继续之前，先读取 task.md、work.md 以及 log.jsonl 中相关的 Self-Driving 记录。",
	suspended:              "此 Self-Driving generation 之前被挂起。请先检查唤醒条件。满足时继续工作；不满足时，只有在没有其他范围内工作可以推进、继续只会轮询该外部条件时才再次挂起，否则继续工作。",
