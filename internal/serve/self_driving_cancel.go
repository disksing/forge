...
type selfDrivingCancelRequest struct {
...
type selfDrivingCancelResponse struct {
	Task            app.Task `json:"task"`
	Interrupted     bool     `json:"interrupted"`
	SessionRetained bool     `json:"sessionRetained"`
}

// cancelChatSelfDriving is the control-plane cancellation endpoint. The task
...
// the driver.
func (s *server) cancelChatSelfDriving(w http.ResponseWriter, r *http.Request, workspaceID string) {
	workspace, err := s.workspace(workspaceID)
	if err != nil {
		writeError(w, err, http.StatusNotFound)
		return
	}
	var req selfDrivingCancelRequest
	...
	if resource.Task == nil {
		writeError(w, errors.New("Self-Driving can only be cancelled on a task"), http.StatusBadRequest)
		return
	}

	task, err := forgeWorkspace.CancelSelfDriving(app.SelfDrivingActionInput{
		TaskID:             resourceID,
	...
	if task.SelfDriving == nil {
		writeError(w, errors.New("Self-Driving cancellation did not return a generation"), http.StatusInternalServerError)
		return
	}

	interrupted := false
	var interruptErrors []error
	for _, rt := range s.agents.selfDrivingRuntimes(resourceID, task.SelfDriving.Generation, strings.TrimSpace(req.RunID)) {
		if err := s.agents.interruptCancelledSelfDrivingRun(r.Context(), rt); err != nil {
	...
	if len(interruptErrors) > 0 {
		writeError(w, fmt.Errorf("Self-Driving was cancelled durably, but its active turn could not be interrupted safely: %w", errors.Join(interruptErrors...)), http.StatusBadGateway)
		return
	}
	writeJSON(w, selfDrivingCancelResponse{Task: task, Interrupted: interrupted, SessionRetained: true})
}

func (m *agentManager) selfDrivingRuntimes(resourceID string, generation int, runID string) []*agentRuntime {
	m.mu.Lock()
	...
	for _, rt := range runtimes {
		...
		if run.ResourceID != resourceID || !run.SchedulerTurn || run.SelfDrivingGeneration != generation {
			continue
		}
...
// interruptCancelledSelfDrivingRun acts only after CancelSelfDriving has committed.
...
func (m *agentManager) interruptCancelledSelfDrivingRun(ctx context.Context, rt *agentRuntime) error {
	rt.turnActionMu.Lock()
	defer rt.turnActionMu.Unlock()
	run := rt.snapshotRun()
	if !run.SchedulerTurn || run.SelfDrivingGeneration <= 0 || strings.TrimSpace(run.AgentHubSessionID) == "" || rt.agentHub == nil {
		return nil
	}
	...
	if err != nil {
		...
		recoveryErr := fmt.Errorf("AgentHub turn state could not be confirmed after Self-Driving cancellation: %w", err)
