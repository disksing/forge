...
function renderSessions() {
  ...
  for (const session of sessions) {
    ...
    row.className = `session-row ${isInternal ? "internal-session" : "external-session"} ${statusPresentation.layoutClassName} ${statusPresentation.className} ${clickable ? "clickable-session" : ""} ${isCurrent ? "current-session" : ""}`;
    ...
    row.setAttribute("aria-label", `${title}. ${statusLabel}. ${providerLabel}`);
    row.innerHTML = `
