...
<section class="attention-section" data-component-owner="attention-list">
  <div class="activity-tabs" role="tablist" aria-label="Activity categories">
    {#each tabs as tab}
      <button type="button" role="tab" aria-selected={activeTab === tab.key} aria-controls={`activity-panel-${tab.key}`} class:active={activeTab === tab.key} onclick={() => { activeTab = tab.key; }}>{tab.label} <span class="activity-tab-count">{activity[tab.key].length}</span></button>
    {/each}
  </div>
  <div id={`activity-panel-${activeTab}`} class="attention-list" role="tabpanel" aria-label={`${tabs.find((tab) => tab.key === activeTab)?.label || "Activity"} resources`}>
