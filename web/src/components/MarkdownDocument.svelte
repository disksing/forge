<script lang="ts">
  ...
  let { file, workspaceId, editable = false, resolveResourceTitle, onNavigate, onSave = async (path) => ({ path }), onToast = () => undefined, onIconsChanged = () => undefined }: { file: ResourceFileModel; workspaceId: string; editable?: boolean; resolveResourceTitle: ResourceTitleResolver; onNavigate: (resourceId: string) => void; onSave?: (path: string, content: string, expectedContentHash: string) => Promise<FilePreviewModel>; onToast?: (message: string) => void; onIconsChanged?: () => void } = $props();
...
<div class="content-section" data-component-owner="markdown-document" data-doc-file={file.name} data-document-identity={`${workspaceId}:${file.path || file.name}:preview:${file.contentHash || "unversioned"}`}>
  ...
  {:else if markdown}<div class="markdown-document-actions">{#if editable}<button type="button" class="secondary-button" onclick={() => editing = true}><Icon name="pencil" /><span>Edit / Annotate</span></button>{/if}</div><div class="markdown-preview"><div class="markdown-view markdown-rendered" use:markdownResourceNavigation={{ resolveResourceTitle, onNavigate }}>{@html markdownHTML(file.content || "", { workspaceId, resolveResourceTitle })}</div></div>
