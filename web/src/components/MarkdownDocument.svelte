<script lang="ts">
  ...
  let { file, workspaceId, editable = false, resolveResourceTitle, onNavigate, onOpenFile, onEdit }: { file: ResourceFileModel; workspaceId: string; editable?: boolean; resolveResourceTitle: ResourceTitleResolver; onNavigate: (resourceId: string) => void; onOpenFile?: (path: string) => void; onEdit?: (path: string) => void } = $props();
...
<div class="content-section" data-component-owner="markdown-document" data-doc-file={file.name} data-document-identity={`${workspaceId}:${file.path || file.name}:preview:${file.contentHash || "unversioned"}`}>
  {#if markdown}<div class="markdown-preview"><div class="markdown-document-actions">{#if editable && onEdit}<button type="button" class="secondary-button" onclick={() => onEdit(file.path || file.name)}><Icon name="pencil" /><span>Edit / Annotate</span></button>{/if}</div><div class="markdown-view markdown-rendered" use:markdownResourceNavigation={{ resolveResourceTitle, onNavigate, onOpenFile }}>{@html markdownHTML(file.content || "", { workspaceId, resolveResourceTitle })}</div></div>
