...
<details data-component-owner="event-timeline" class="agent-tool-group" data-tool-group-key={`${generationId}:${String(item.key || item.time || "tools")}`} {open} ontoggle={(event) => onToggle(event.currentTarget.open)}>
