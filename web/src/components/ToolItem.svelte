<script lang="ts">
  ...
  let { call, open = false }: { call: ToolCall; open?: boolean } = $props();
...
<details data-component-owner="event-timeline" class={`agent-tool-item agent-tool-${String(call.status || "completed")}`} {open}>
