...
function summarizeApproval(event) {
  ...
  const options = Array.isArray(data.options)
    ...
    : [];
  return { title: firstString(data.title, "Approval requested"), detail: firstString(data.detail), question: firstString(data.question), options };
}
  ...
  for (const event of events || []) {
    ...
    switch (type) {
      ...
      case "message.user.steer":
        ...
        items.push({
          kind: "message", role: "user", key: timelineEventKey(event), time,
      ...
      case "message.assistant.delta": {
        ...
        if (last?.kind === "message" && last.role === "assistant" && last.turnId === turnId) {
          ...
          items.push({ kind: "message", role: "assistant", key: timelineEventKey(event), turnId, text, time });
        }
        break;
      }
      case "message.reasoning.delta": {
        ...
        if (openThinking) {
          ...
          openThinking = {
            kind: "thinking", key: timelineEventKey(event), turnId, text,
      ...
      case "tool.call": {
        ...
        if (existing) {
          ...
          const targetGroup = group || { kind: "tools", key: timelineEventKey(event), calls: [], time };
      ...
      case "approval.requested": {
        ...
        const item = {
          kind: "approval", key: timelineEventKey(event), time,
      ...
      case "approval.resolved": {
        ...
        if (target) {
          ...
          items.push({
            kind: "approval", key: timelineEventKey(event), time, approvalId,
      ...
      case "provider.error":
        {
          ...
          if (data.retryable === true) {
            items.push({
              kind: "lifecycle", tone: "info", key: timelineEventKey(event), time,
              text,
            });
          } else {
            items.push({ kind: "error", key: timelineEventKey(event), time, text });
          }
        }
        break;
      case "turn.started":
        openThinking = null;
        items.push({ kind: "lifecycle", tone: "muted", key: timelineEventKey(event), time, text: "Turn started" });
        break;
      case "turn.completed":
        openThinking = null;
        settleOpenToolCalls("completed", time);
        items.push({ kind: "lifecycle", tone: "ok", key: timelineEventKey(event), time, text: "Turn completed" });
        break;
      case "turn.failed":
        openThinking = null;
        settleOpenToolCalls("failed", time);
        items.push({
          kind: "lifecycle", tone: "danger", key: timelineEventKey(event), time,
      ...
      case "turn.cancelled":
        openThinking = null;
        settleOpenToolCalls("failed", time);
        items.push({ kind: "lifecycle", tone: "muted", key: timelineEventKey(event), time, text: "Turn interrupted" });
        break;
      case "session.created":
        items.push({ kind: "lifecycle", tone: "muted", key: timelineEventKey(event), time, text: "Session created" });
        break;
      case "session.provider": {
        ...
        items.push({ kind: "lifecycle", tone: "muted", key: timelineEventKey(event), time, text: parts.join(" · ") });
        break;
      }
      case "session.state": {
        ...
        if (label) items.push({ kind: "lifecycle", tone: danger ? "danger" : "muted", key: timelineEventKey(event), time, text: label });
        break;
      }
      case "session.archived":
        openThinking = null;
        items.push({ kind: "lifecycle", tone: "muted", key: timelineEventKey(event), time, text: "Session archived" });
        break;
      default: {
        if (isActivityType(type)) {
          ...
          items.push({
            kind: "unknown", key: timelineEventKey(event), time, type: type || "unknown",
