...
function renderCreateDialog() {
  ...
  const renderKey = `${dialog.type}:${dialog.projectId}:${dialog.templateName}:${dialog.autorun}:${dialog.submitting}:${dialog.previewing}:${dialog.titleOverride}:${JSON.stringify(dialog.templateFields)}:${dialog.templateDigest}:${JSON.stringify(dialog.preview)}`;
      ...
      <section class="create-dialog${isTask ? " create-task-dialog" : ""}${entering ? " modal-enter" : ""}" role="dialog" aria-modal="true" aria-label="${title}">
        ...
        <form id="createDialogForm" class="details-form create-dialog-form">
          ${isTask ? `
            ...
            <label>
              <span>Task title ${selectedTemplate && !dialog.titleOverride ? "<small>(generated by template)</small>" : ""}</span>
              <span class="template-title-control">
                <input name="title" ${selectedTemplate ? "" : "required"} value="${escapeHTML(selectedTemplate ? shownTitle : dialog.title)}" placeholder="${selectedTemplate ? "Generated after valid fields are entered" : "Task title"}" />
                ${selectedTemplate && dialog.titleOverride ? `<button type="button" class="secondary compact" data-clear-title-override="true">Use generated</button>` : ""}
              </span>
            </label>
            ${selectedTemplate ? `
              <div class="template-fields" aria-label="Template fields">
                ${(selectedTemplate.fields || []).map((field) => templateFieldControl(field, dialog.templateFields[field.name])).join("")}
              </div>
            ...
            ${selectedTemplate ? `
              <div class="template-preview-actions">
                <button type="button" class="secondary" data-create-preview="true" ${dialog.previewing ? "disabled" : ""}>${dialog.previewing ? "Rendering..." : "Preview"}</button>
                ${dialog.templateDigest ? `<small>Template ${escapeHTML(dialog.templateName)} · ${escapeHTML(dialog.templateDigest)}</small>` : ""}
              </div>
              ${dialog.preview ? `
                <section class="template-preview" aria-label="Task preview">
                  <h4>${escapeHTML(dialog.preview.title)}</h4>
                  <div class="markdown-body">${renderMarkdown(dialog.preview.markdown)}</div>
...
let createTaskPreviewTimer = 0;

function scheduleCreateTaskPreview() {
  window.clearTimeout(createTaskPreviewTimer);
  createTaskPreviewTimer = window.setTimeout(() => {
    if (state.createDialog.open && state.createDialog.templateName && !state.createDialog.submitting) {
      refreshCreateTaskPreview(true).catch(() => {});
    }
  }, 350);
}
...
function bindCreateDialogEvents() {
  ...
  form.addEventListener("input", (event) => {
    ...
    if (["slug", "prompt", "agentName", "completionCriteria", "agentProfiles"].includes(target.name) && state.createDialog.templateName) {
      scheduleCreateTaskPreview();
    }
    if (target.name === "autorun") {
      state.createDialog.autorun = target.checked;
      renderCreateDialog();
      if (state.createDialog.templateName) scheduleCreateTaskPreview();
    }
  });
  ...
  document.querySelector("[data-create-preview]")?.addEventListener("click", () => refreshCreateTaskPreview(false).catch((err) => toast(err.message)));
...
function applyCreateDialogTemplate(name) {
  const dialog = state.createDialog;
	if (name !== dialog.templateName && (dialog.templateDirty || dialog.titleOverride)) {
  ...
  if (template) {
	...
	dialog.templateDigest = "";
  renderCreateDialog();
	if (template) scheduleCreateTaskPreview();
}
...
async function refreshCreateTaskPreview(silent = false) {
  const dialog = state.createDialog;
  ...
  try {
    if (dialog.type === "project") {
      ...
      state.selectedId = "workspace";
		...
		if (dialog.templateName && !dialog.templateDigest) await refreshCreateTaskPreview(false);
      await api(`/api/workspaces/${state.activeWorkspaceId}/tasks`, {
        method: "POST",
        body: JSON.stringify(createTaskRequest(dialog)),
      });
      toast("Task created.");
    }
