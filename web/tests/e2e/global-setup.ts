...
// the responding pua instance actually belongs to this e2e run instead of
// silently running tests against an unrelated local pua instance.
const runtimeDirMarker = `pua-frontend-e2e-${port}`;
...
export default async function globalSetup(): Promise<void> {
  ...
  try {
    ...
    throw new Error(
      `pua e2e server at ${baseURL} is not reachable after Playwright reported it ready: ${reason instanceof Error ? reason.message : String(reason)}`,
    );
  }
  if (!response.ok) {
    throw new Error(`pua e2e server at ${baseURL} answered /api/workspaces with HTTP ${response.status}; refusing to run e2e tests against an unrecognized instance.`);
  }
  ...
  throw new Error([
    `Port ${port} is occupied by a pua instance that does not belong to this e2e run.`,
    ...
    `  PUA_E2E_PORT=<free-port> npm run test:e2e`,
  ].join("\n"));
}
