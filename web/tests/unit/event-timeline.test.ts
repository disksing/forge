...
import type { AgentEvent, AgentTurnItem, EventTimelineModel, TimelineItem } from "../../src/components/models";
...
function model(resourceId: string, projector: (events: AgentEvent[]) => TimelineItem[] = project): EventTimelineModel {
  return {
...
describe("EventTimeline", () => {
  ...
  it("keeps compact tool counts and expands to the same group in Chat and History", async () => {
    ...
    const chat = mount(EventTimeline, { target: chatTarget, props: { channel: createModelChannel(model("task-a", projectConversationEvents)) } });
