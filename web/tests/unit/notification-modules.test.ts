...
describe("notification modules", () => {
  it("normalizes persisted records and rejects malformed entries", () => {
    const store = normalizeNotificationStore({ version: 1, seen: [{ marker: " done ", at: 5 }, {}], pending: [{ marker: "m1", generationId: "gen-1", at: 10 }, { marker: "missing-generation" }], unread: "bad", effects: [{ key: "sound:m1", at: 11 }] });
    expect(store.seen).toEqual([{ marker: "done", at: 5 }]);
    expect(store.pending).toHaveLength(1);
    expect(store.pending[0]).toMatchObject({ marker: "m1", generationId: "gen-1", completionState: "completed" });
  ...
  it("projects completion identity and display content", () => {
    const source = { id: "gen-1", resourceId: "task1", generationId: "gen-1", completionEventId: 42 };
    expect(notificationMarkerFor(source)).toBe("gen-1:42");
    expect(notificationEventState({ id: 42, type: "turn.failed" })).toBe("failed");
    const record = createNotificationRecord(source, {
      workspaceId: "workspace-a", marker: "gen-1:42", completionState: "failed",
      findResource: () => ({ id: "task1", type: "task", title: "Build release" }),
      now: () => 100,
    });
    expect(record).toMatchObject({ workspaceId: "workspace-a", generationId: "gen-1", resourceId: "task1", at: 100 });
